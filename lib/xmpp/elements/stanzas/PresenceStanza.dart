...
class PresenceStanza extends AbstractStanza {
  ...
  String? get status {
    var statusElement =
        children.firstWhere((element) => element?.name == 'status' && element?.attributes.isEmpty == true, orElse: () => null);
  ...
  int? get priority {
    var a = getChild('priority')?.textValue;
    if (a == null) {
      return null;
    }
  ...
  void _setChildValue(String childName, String value) {
    var childElement =
        children.firstWhere((element) => element?.name == childName && element?.attributes.isEmpty == true, orElse: () => null);
    if (childElement == null) {
      var element = XmppElement();
